...
In questa nota approfondiamo alcuni costrutti sintattici fighi che [Haskell](content/Informatica/Lambda-calcolo/Haskell/_index.md#^definizione-haskell) ha per scrivere le proprie [funzioni](Sintassi%20di%20base%20di%20Haskell.md#^definizione-funzione-in-haskell), tra cui il pattern matching%% link %%.

# Pattern matching

> [!definizione] Definizione: pattern matching
> 
> In [Haskell](content/Informatica/Lambda-calcolo/Haskell/_index.md#^definizione-haskell) il **pattern matching** è un costrutto sintattico%% link %% usato nel [corpo di una funzione](Sintassi%20di%20base%20di%20Haskell.md#^definizione-funzione-in-haskell) per selezionare un ramo di esecuzione%% link %% della [funzione](Sintassi%20di%20base%20di%20Haskell.md#^definizione-funzione-in-haskell) a seconda della forma dell'argomento%% link %% (detto **pattern**) che le viene passato:
> 
> ```haskell
> describe :: Int -> String
> describe 0 = "zero"
...
> ```haskell
> testa :: [a] -> a
> testa []    = error "lista vuota"
> testa (x:_) = x
> ```
^definizione-pattern-matching

%% 
mettere un esempio migliore di `testa`
%%

%% 
al massimo spostare la definizione del pattern wildcard in un'altra definizione
%%

> [!osservazione] Osservazione: codice pulito col pattern matching
> 
> This leads to really neat code that’s simple and readable.Without pattern matching, we’d have to make a pretty convoluted if then else tree.

> [!osservazione] Osservazione: è importante l'ordine dei pattern
> 
> Essendo dall'alto verso il basso, se mettiamo la variabile o la wildcard all'inizio verrà sempre eseguito quello:
> 
> ```haskell
> describe :: Int -> String
> describe n = "something else"
...
> ```
> 
> in questo caso uscirà sempre `"something else"`

> [!osservazione] Osservazione: ricorsione col pattern matching
> 
...
> [!attenzione] Attenzione: pattern matching non esaustivo
> 
...
> [!osservazione] Osservazione: pattern matching nelle tuple
> 
...
> [!osservazione] Osservazione: pattern matching con le triple
> 
...
> [!osservazione] Osservazione: pattern matching nelle list comprehension
> 
...
> [!osservazione] Osservazione: liste nel pattern matching
> 
...
> [!notazione] Notazione: pattern `x:xs`
> 
...
> 
> `xs` va inteso come "`x`s" (nel senso di "multipli `x`" in inglese).

> [!esempio] Esempio: implementazione della funzione `head` col pattern matching
> 
...
> [!esempio] Esempio: funzione `tell` col pattern matching
> 
...
> [!esempio] Esempio: implementazione della funzione `length` col pattern matching
> 
...
> [!esempio] Esempio: implementazione della funzione `sum` col pattern matching
> 
...
> [!definizione] Definizione: as patterns
> 
...
> [!attenzione] Attenzione: non si può usare `++` nel pattern matching
> 
...
>   | voto >= 70 = "buono"
>   | voto >= 60 = "sufficiente"
>   | otherwise  = "insufficiente"
> ```
> 
> Le **guardie** vengono valutate dall'alto verso il basso e viene scelta il primo ramo di esecuzione%% link %% la cui condizione%% link %% risulta `True`. La guardia `otherwise` funge da caso di default e viene automaticamente valutata come `True`.

%% 
esempio nella definizione in inglese
%%

> [!attenzione] Attenzione: ordine delle guardie
> 
> anche qui è importante l'ordine delle guardie

> [!attenzione] Attenzione: no `=` nelle guardie
> 
...
> ```
> 
> Guards are indicated by pipes that follow a function’s name and its parameters. Usually, they’re indented a bit to the right and lined up. A guard is basically a boolean expression. If it evaluates to `True`, then the corresponding function body is used. If it evaluates to `False`, checking drops through to the next guard and so on. If we call this function with `24.3`, it will first check if that’s smaller than `1.2`. Because it isn’t, it falls through to the next guard. The check is carried out with the second guard and because `24.3` is less than `1000.0`, the second string is returned.

> [!osservazione] Osservazione: codice più leggibile
> 
> Anche qui vale la stessa osservazione fatta prima per il pattern matching
> 
...
> [!osservazione] Osservazione: affinità e divergenze tra il pattern matching e le guardie
> 
...
> [!esempio] Esempio: guardia a più parametri
> 
...
> [!esempio] Esempio: implementazione della funzione `max` con le guardie
> 
...
> ```
> 
> Non molto leggibile però, meglio scrivere su più righe

> [!esempio] Esempio: implementazione della funzione `compare` con le guardie
> 
> ```haskell
...
> [!osservazione] Osservazione: `where` col pattern matching
> 
> You can also use `where` bindings to **pattern match**! We could have rewritten the `where` block of our previous function as:
> 
> ```haskell
> where density = mass / volume  
...
> [!osservazione] Osservazione: usare `where` per definire funzioni
> 
...
> [!osservazione] Osservazione: `where` innestati
> 
> `where` bindings can also be nested. It’s a common idiom to make a function and define some helper function in its `where` block and then to give those functions helper functions as well, each with its own `where` block.
...
> 
> In [Haskell](content/Informatica/Lambda-calcolo/Haskell/_index.md#^definizione-haskell) **`let in`** è un costrutto sintattico%% link %% usato per definire variabili%% link %% o funzioni%% link %% locali visibili esclusivamente nell'espressione%% link %% che segue la parola chiave `in`.

%% 
esempio nella definizione
%%

> [!osservazione] Osservazione: affinità e divergenze tra `where` e `let in`
> 
...
> [!osservazione] Osservazione: usare `let in` nelle espressioni inline
> 
...
> [!osservazione] Osservazione: `let in` nella list comprehension
> 
...
> [!osservazione] Osservazione: perché non usare `let in` al posto di `where`?
> 
...
%% 
inserire definizione delle case expressions
%%
...
> 	- [4 - _Syntax in Functions_](https://learnyouahaskell.github.io/syntax-in-functions.html).
