...
[Nix](Nix.md#^definizione-package-manager-nix) è il punto d'ingresso a un _rabbit hole_ in cui sono cascato ormai anni fa e da cui non ne sono più uscito, complice anche il fatto che da [Nix](Nix.md#^definizione-package-manager-nix) deriva il [sistema operativo _NixOS_](NixOS.md#^definizione-nixos) che è tutt'ora il [sistema operativo](Sistemi%20operativi.md#^definizione-sistema-operativo) che principalmente uso.
...
Per farlo, Dolstra%% link %% utilizza un semplice ma geniale trucco: ogni pacchetto%% link %% viene isolato in una cartella%% link %% a se stante e, anziché conservare il pacchetto%% link %% vero e proprio, il [sistema operativo](Sistemi%20operativi.md#^definizione-sistema-operativo) si salva una sua "descrizione", detta [_derivazione_](Nix.md#^definizione-derivazione), che specifica come costruire il pacchetto%% link %%, incluse le sue [dipendenze](Informatica.md#^definizione-dipendenza).

Le [derivazioni](Nix.md#^definizione-derivazione) sono espresse (o meglio, _dichiarate_) sotto forma di funzioni%% link %% che prendono in input le [dipendenze](Informatica.md#^definizione-dipendenza) del pacchetto%% link %% in questione e il cui risultato è l'installazione%% link %% del pacchetto%% link %% nel [sistema operativo](Sistemi%20operativi.md#^definizione-sistema-operativo).
...
> 
> In [Nix](Nix.md#^definizione-package-manager-nix), una **derivazione** è una descrizione dichiarativa%% link %% della build di un pacchetto%% link %%, espressa sotto forma di una funzione%% link %% che prende come input%% link %% le [dipendenze](Informatica.md#^definizione-dipendenza) del pacchetto%% link %% e il cui risultato è l'installazione%% link %% del pacchetto%% link %% nel [sistema operativo](Sistemi%20operativi.md#^definizione-sistema-operativo).
> 
...
- isolamento: ogni derivazione è isolata dall'altra
	...
	- L'isolamento permette anche di avere più versioni dello stesso pacchetto, che normalmente sarebbe difficile da ottenere con i package manager tradizionali. Ciò risolve il [problema dell'interferenza tra dipendenze](Distribuzione%20del%20software.md#^problema-dell-interferenza-tra-dipendenze), perché [Nix](Nix.md#^definizione-package-manager-nix) ci permette di avere due versioni separate delle stesse [dipendenze](Informatica.md#^definizione-dipendenza) per i diversi [software](Informatica.md#^definizione-software) che le richiedono.
...
> 
> Come si comporta [Nix](Nix.md#^definizione-package-manager-nix) per le [dipendenze](Informatica.md#^definizione-dipendenza)? [Nix](Nix.md#^definizione-package-manager-nix) si assicura di collegare a ogni [derivazione](Nix.md#^definizione-derivazione) le proprie [dipendenze](Informatica.md#^definizione-dipendenza), a loro volta gestite tramite altre [derivazioni](Nix.md#^definizione-derivazione).
> 
...
> 
> In altre parole, [Nix](Nix.md#^definizione-package-manager-nix) offre solo [dipendenze](Informatica.md#^definizione-dipendenza) "dirette" da una [derivazione](Nix.md#^definizione-derivazione) a un'altra [derivazione](Nix.md#^definizione-derivazione).
