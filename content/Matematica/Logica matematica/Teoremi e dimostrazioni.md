...
%% perché è un esempio il principio di Archimede %%
...
%% 
Affermazione $\supsetneq$ Proposizione $\supsetneq$ (teorema, corollario, lemma)
%%

> [!definizione]+ Definizione: proposizione logica
> 
> Una **proposizione logica** (o, più semplicemente, **proposizione**) è un'affermazione (riguardante numeri, funzioni, enti geometrici, o altri oggetti matematici) che si vuole dimostrare, solitamente enunciato nella forma:
> $$
> \text{Se valgono $P_1, P_2, \ldots, P_n$, allora vale anche $Q$}
> $$
> (dove le affermazioni $P_1, P_2, \ldots, P_n$ sono dette **_ipotesi_**, mentre $Q$ è detta **_tesi_**).
^definizione-proposizione-logica
...
> [!osservazione]+ Osservazione: differenza tra i termini _proposizione_, _teorema_, _lemma_, _corollario_ e _congettura_
> 
> In matematica sono di uso comune anche altri sinonimi di _proposizione_, ad esempio _teorema_, _lemma_, _corollario_, _congettura_ e così via. Dal punto di vista tecnico, tutti questi termini vogliono dire esattamente la stessa cosa, ovvero che quel che si sta affermando è un teorema.
> 
...
> - Il termine _teorema_ per riferirsi a tutte quelle affermazioni che si ritengono veramente importanti e significative.
> - Il termine _lemma_ per quelle affermazioni che di per se non sarebbero molto significative, ma che servono poi per dimostrare altri fatti più importanti.
> - Il termine _corollario_ per quelle affermazioni che dipendono direttamente da un altro teorema o un'altra proposizione.
> - Il termine _proposizione_ in qualsiasi altra affermazione dimostrata.
> - Il termine _congettura_ per quelle affermazioni per cui non è ancora stata trovata una dimostrazione rigorosa.

%% aggiungere "proprietà" %%

In alcuni casi, una stessa implicazione potrebbe essere chiamata _corollario_ in un libro e _proprietà_ in un altro, a seconda dell'enfasi o del pubblico.
...
%%
La differenza concreta tra conseguenza logica $\vDash$ e implicazione logica $\implies$ è che la conseguenza logica $\vDash$ si interessa solo dei casi in cui la premessa (ipotesi) è vera per verificare se la conseguenza (tesi) è vera o falsa.
Se tutti i casi in cui la premessa è vera risulta che anche la conseguenza è vera, allora la conseguenza logica è confermata.

Esempio con $P \vDash Q$:

| Ipotesi $P$ | Tesi $Q$ | $P \overset{?}{\vDash} Q$ |
| ----------- | -------- | ------------- |
| $V$         | $V$      | ✅             |
| $V$         | $F$      | ❌             |
| $F$         | $V$      | ➖             |
...
Per completare la tabella, dobbiamo prima chiederci "ci interessa questo caso?" osservando se l'ipotesi è vera o falsa: solo se e vera, possiamo procedere e vedere se la relazione è rispettata.
In particolare:
- Nella prima riga l'ipotesi $P$ è vera e la tesi $Q$ è anch'essa vera, quindi la conseguenza logica qui è rispettata.
- Nella seconda riga l'ipotesi $P$ è vera ma la tesi $Q$ è falsa, quindi la conseguenza logica qui NON è rispettata.
- Nella terza e quarta riga le ipotesi $P$ sono false, quindi non ci interessano.

Prendendo quindi in considerazione solo le righe che ci interessano (cioè quelle con l'ipotesi $P$ vera), notiamo che non tutte rispettano la conseguenza logica (in particolare la seconda). Si può quindi concludere che $Q$ NON è conseguenza logica di $P$:
$$
P \not\vDash Q
$$
%%
...
## 2.1 - Proprietà della conseguenza logica

> [!proprieta]+ Proprietà riflessiva della conseguenza logica
> 
> Data un'affermazione $P$, si ha che:
> $$
> P \vDash P
> $$
...
> [!proprieta]+ Proprietà transitiva della conseguenza logica
> 
> Date tre affermazioni $P$, $Q$ ed $R$ qualsiasi, si ha che:
> 
> $$
> (P \vDash Q) \land (Q \vDash R) \implies P \vDash R
> $$
...
> 
> Segue dal fatto che $P \vDash R$ può essere dimostrato componendo le dimostrazioni di $P \vDash Q$ e $Q \vDash R$ (dimostrazione per composizioneLINK).

%%

# 3 - Dimostrare una proposizione logica

In matematica, per stabilire la verità di una [proposizione logica](Teoremi%20e%20dimostrazioni.md#^definizione-proposizione-logica) si deve ricorrere a una _dimostrazione_.

> [!definizione]+ Definizione: dimostrazione
> 
> Una **dimostrazione** di una [proposizione logica](Teoremi%20e%20dimostrazioni.md#^definizione-proposizione-logica) è una catena di ragionamenti che ci permette di concludere che la tesi $Q$ deve essere vera partendo dall'assunzione che le ipotesi $P_1, P_2, \ldots, P_n$ siano vere.
...
## 3.1 - Contesto di un'affermazione

La validità di una [proposizione logica](Teoremi%20e%20dimostrazioni.md#^definizione-proposizione-logica) o, più in generale, di un'affermazione%% link %% dipende dal suo contesto.
...
## 3.2 - Tautologie e contraddizioni
...
> 
> Una **tautologia** è un'affermazione sempre vera in qualunque [contesto](Teoremi%20e%20dimostrazioni.md#^definizione-contesto-di-un-affermazione), ossia un enunciato per il quale:
> $$
> \vDash Q
> $$
...
%% 
Osservazione: si scrive così la tautologia perché si intende che come ipotesi ci sia sempre una verità, quindi di conseguenza si può prendere sempre in considerazione $Q$.
%%
...
> [!definizione]+ Definizione: contraddizione
> 
> Una **contraddizione** è una proposizione sempre falsa.
^definizione-contraddizione
...
%% enunciare il principio dell'ex falso quodlibet %%

# 4 - Tipi di dimostrazioni
...
## 4.3 - Dimostrazione per contrapposizione
...
## 4.4 - Dimostrazione per composizione
...
^definizione-dimostrazione-per-casi
...
## 4.6 - Combinazione delle dimostrazioni
...
>     - Corso di _Logica Matematica_, A.A. 2025-26 ([pagina Moodle](https://informatica.i-learn.unito.it/course/view.php?id=3561)):
> 		- Prof. Viale Matteo, slide del corso:
> 			- [1.1 - _Teoremi e dimostrazioni_](https://informatica.i-learn.unito.it/pluginfile.php/417200/mod_folder/content/0/1.1%20-%20Teoremi%20e%20dimostrazioni_moodle.pdf).
> 			- [1.2 - _Simboli logici_](https://informatica.i-learn.unito.it/pluginfile.php/417200/mod_folder/content/0/1.2%20-%20Simboli%20logici_moodle.pdf).
